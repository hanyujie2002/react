---
title: '响应式 Effect 的生命周期 | Lifecycle of Reactive Effects'
---
...
## Effect 的生命周期 | The lifecycle of an Effect {/*the-lifecycle-of-an-effect*/}
...
### 为什么同步可能需要多次进行 | Why synchronization may need to happen more than once {/*why-synchronization-may-need-to-happen-more-than-once*/}
...
### React 如何重新同步 Effect | How React re-synchronizes your Effect {/*how-react-re-synchronizes-your-effect*/}
...
### 从 Effect 的角度思考 | Thinking from the Effect's perspective {/*thinking-from-the-effects-perspective*/}
...
### React 如何验证 Effect 可以重新进行同步 | How React verifies that your Effect can re-synchronize {/*how-react-verifies-that-your-effect-can-re-synchronize*/}
...
### React 如何知道需要重新进行 Effect 的同步 | How React knows that it needs to re-synchronize the Effect {/*how-react-knows-that-it-needs-to-re-synchronize-the-effect*/}
...
### 每个 Effect 表示一个独立的同步过程。| Each Effect represents a separate synchronization process {/*each-effect-represents-a-separate-synchronization-process*/}
...
## Effect 会“响应”于响应式值 | Effects "react" to reactive values {/*effects-react-to-reactive-values*/}
...
### 没有依赖项的 Effect 的含义 | What an Effect with empty dependencies means {/*what-an-effect-with-empty-dependencies-means*/}
...
### 在组件主体中声明的所有变量都是响应式的 | All variables declared in the component body are reactive {/*all-variables-declared-in-the-component-body-are-reactive*/}
...
#### 全局变量或可变值可以作为依赖项吗？| Can global or mutable values be dependencies? {/*can-global-or-mutable-values-be-dependencies*/}
...
### React 会验证是否将每个响应式值都指定为了依赖项 | React verifies that you specified every reactive value as a dependency {/*react-verifies-that-you-specified-every-reactive-value-as-a-dependency*/}
...
### 当你不想进行重新同步时该怎么办 | What to do when you don't want to re-synchronize {/*what-to-do-when-you-dont-want-to-re-synchronize*/}
...
#### 修复每次输入均重新连接 | Fix reconnecting on every keystroke {/*fix-reconnecting-on-every-keystroke*/}
...
#### 打开和关闭状态同步 | Switch synchronization on and off {/*switch-synchronization-on-and-off*/}
...
#### 寻找过时值的错误 | Investigate a stale value bug {/*investigate-a-stale-value-bug*/}
...
#### 修复连接开关 | Fix a connection switch {/*fix-a-connection-switch*/}
...
#### 填充一系列选择框 | Populate a chain of select boxes {/*populate-a-chain-of-select-boxes*/}
  ...
  useEffect(() => {
    ...
    fetchData('/planets').then(result => {
      if (!ignore) {
        ...
        setPlanetId(result[0].id); // 选择第一个行星 // Select the first planet
      }
    });
    return () => {
      ignore = true;
  ...
  useEffect(() => {
    ...
    fetchData('/planets/' + planetId + '/places').then(result => {
      if (!ignore) {
        ...
        setPlaceList(result);
        setPlaceId(result[0].id); // 选择第一个地点 // Select the first place
      }
    });
    return () => {
      ignore = true;
